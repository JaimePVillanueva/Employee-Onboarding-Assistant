...
def guardar_dia (estado: EstadoConversacion):
    ...
    ruta= f"dias/dia{estado.empleado_id}_dia_{estado.dia}.json"
...
def cargar_dia_amterior (empleado_id:str, dia_int) -> dict | None:
...
def tareas_pendientes (empleado_id :str , dia:int) ->list:
    ...
    retunr [t for t in dia_anterior ["tareas"] if not t["completado"]]